...
const COUNCIL_TOOL = {
  name: "council_run",
  description:
    "Run a true multi-session council review using isolated Copilot runs for persona generation, parallel reviewers, judge synthesis, and recommendation audit.",
  inputSchema: {
    type: "object",
    properties: {
      subject: {
      ...
      subjectLabel: {
        type: "string",
        description: "Short label used in the council output and artifact file."
      },
      personaCount: {
        type: "integer",
      ...
      model: {
        type: "string",
        description:
          "Optional model identifier for spawned Copilot runs. Defaults to the configured CLI default model."
      },
      workingDirectory: {
        type: "string",
        description:
          "Optional working directory for resolving subjectPaths and artifactPath."
      },
      artifactPath: {
        type: "string",
        description:
          "Optional markdown artifact file path. If omitted, the plugin writes into the Copilot home directory."
      }
    }
  }
};
...
async function handleMessage(message) {
  ...
  try {
    switch (method) {
      case "initialize": {
        negotiatedProtocolVersion =
          normalizeOptionalString(params?.protocolVersion) || DEFAULT_PROTOCOL_VERSION;
        sendResult(id, {
    ...
    sendError(id, -32000, toErrorMessage(error));
  }
}

async function handleToolCall(params) {
  const toolName = normalizeOptionalString(params?.name);
  if (toolName !== COUNCIL_TOOL.name) {
    throw new Error(`Unknown tool: ${toolName || "missing name"}`);
  }

  const result = await runCouncil(params?.arguments, { source: "mcp" });
  return {
    content: [
      {
        type: "text",
...
async function runCouncil(rawArgs, { source }) {
  ...
  const currentModel = args.model || "configured default";

  try {
    const subjectBundle = await buildSubjectBundle(args, cwd);
    log(`Preparing ${args.personaCount} reviewer slots for "${subjectBundle.label}"`);

    const councilRunId = shortId();
    const personaEnvelope = await generatePersonas({
      councilRunId,
      cwd,
      model: args.model,
      subjectBundle,
    ...
    log(`Spawned ${personas.length} grounded reviewers`);
    ...
    if (successfulReviews.length < MIN_PERSONA_COUNT) {
      const artifactPath = await writeArtifact(
        ...
        buildArtifact({
          ...
          judgeVerdict: null,
          judgeError:
            "Judge phase skipped because fewer than two reviewer runs succeeded.",
          auditVerdict: null,
          auditError:
      ...
      return failureResult(
        [
          ...
          `Reviewer runs succeeded: ${successfulReviews.length}/${personas.length}.`,
    ...
    try {
      ...
      log("Recommendation audit complete");
    } catch (error) {
    ...
    return failureResult(`Council orchestration failed: ${toErrorMessage(error)}`);
  }
}
...
async function buildSubjectBundle(args, cwd) {
  ...
  if (args.subject) {
    const trimmed = trimToLimit(
      args.subject,
      Math.min(remainingBudget, DEFAULT_MAX_TOTAL_CHARS)
    );
  ...
  for (const rawPath of args.subjectPaths) {
    ...
    if (trimmed.truncated) {
      reviewLimits.push(`File content was truncated for council review: ${rawPath}`);
    }

    remainingBudget -= trimmed.text.length;
    sections.push(
      [`## File: ${rawPath}`, "```text", trimmed.text, "```"].join("\n")
    );
  }
  ...
  const label =
    args.subjectLabel || deriveLabel(args.subject, args.subjectPaths) || "Council subject";
...
async function generatePersonas(options) {
  let lastError = null;

  for (let attempt = 1; attempt <= 2; attempt += 1) {
    const raw = await runCopilotPhase({
      cwd: options.cwd,
      model: options.model,
      systemPrompt: personaGeneratorPrompt(),
...
async function runReviewer(options) {
  ...
  try {
    const content = await runCopilotPhase({
      cwd: options.cwd,
      model: options.model,
      systemPrompt: reviewerPrompt(persona),
      userPrompt: reviewerUserPrompt(options.subjectBundle, options.inheritedLimits)
    });

    log(`Reviewer ${options.index + 1}/${options.total} complete: ${persona.name}`);
    ...
    log(`Reviewer failed: ${persona.name} - ${message}`);
    return {
      ok: false,
      persona,
      error: message
...
async function runCopilotPhase(options) {
  const prompt = [
    "You are an internal worker in a council orchestration pipeline.",
    "Complete the task from the provided packet only.",
  ...
  const result = await spawnCopilot(args, options.cwd);
  if (!result.stdout) {
    throw new Error("Copilot phase returned no content.");
  }

  return result.stdout.trim();
}

async function spawnCopilot(args, cwd) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      finished = true;
      child.kill("SIGTERM");
      reject(new Error("Timed out waiting for a Copilot phase to finish."));
    }, COPILOT_PHASE_TIMEOUT_MS);
    ...
    child.on("error", (error) => {
      ...
      reject(new Error(`Failed to start Copilot CLI: ${toErrorMessage(error)}`));
    });
    ...
    child.on("close", (code, signal) => {
      ...
      const detail = stderr.trim() || stdout.trim() || `exit code ${code || signal || "unknown"}`;
      reject(new Error(`Copilot phase failed: ${detail}`));
    });
  });
}
...
async function defaultArtifactPath() {
  ...
  const artifactDir = path.join(copilotHome, "files", "council-runs");
...
function buildSummary({
  ...
  return [
    ...
    `Successful reviewer runs: ${successfulReviews.length}/${personas.length}`,
    "",
    "## Persona Roster",
    personas
      .map(
